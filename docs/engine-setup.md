...
const engineConfig = {
  ...
  performanceMonitor: process.env.NEXT_PUBLIC_ENGINE_PERFORMANCE_MONITOR === 'true',
  ...
  return (
    <div>
      ...
      <Engine.Canvas>
        <Engine.Core>
          <InteractionSystem 
            onObjectClick={engineConfig.enableAnalytics ? trackClick : undefined}
          />
          <RoomScene />
        </Engine.Core>
      </Engine.Canvas>
...
export default function ThemedViewer({ theme = 'dark' }: { theme: keyof typeof THEMES }) {
  const currentTheme = THEMES[theme];

  return (
    <Engine.Canvas engineSettings={{ backgroundColor: currentTheme.background }}>
      <Engine.Core>
        <CameraSystem 
...
export const EngineConfigProvider = ({ children, config }: { 
...
const useFeatureFlags = (): FeatureFlags => {
  return {
    ...
    enhancedInteractions: process.env.NEXT_PUBLIC_FF_ENHANCED_INTERACTIONS === 'true',
    experimentalRendering: process.env.NEXT_PUBLIC_FF_EXPERIMENTAL_RENDERING === 'true'
  };
};
...
export default function FeatureFlaggedViewer() {
  ...
  return (
    <Engine.Canvas>
      <Engine.Core>
        {flags.newCameraControls ? (
          <EnhancedCameraSystem />
        ) : (
          <CameraSystem />
        )}
        
        <InteractionSystem 
          enhancedMode={flags.enhancedInteractions}
        />
        
        <RoomScene 
          experimentalRendering={flags.experimentalRendering}
        />
      </Engine.Core>
    </Engine.Canvas>
  );
