...
export type TimelineItem = { id: number; date: string; title: string; active?: boolean };
...
export default function Sidebar({
    ...
    const initialSelected = initialSelectedId ?? timeline.find((t) => t.active)?.id ?? (timeline.length ? timeline[0].id : undefined);
        ...
        <Card className="col-span-12 md:col-span-4 xl:col-span-4 p-6 h-[88vh] overflow-y-auto text-[15px]">
            ...
            <div className="mt-5">
                <motion.button
                    ...
                    aria-disabled={ctaDisabled || !selectedItem}
                >
                    {ctaText}
                    <AnimatePresence initial={false}>
                        {ctaPressed ? (
                            <motion.span
                                key="ok"
                                initial={{ opacity: 0, rotate: -15, y: 6 }}
                                animate={{ opacity: 1, rotate: 0, y: 0 }}
                                exit={{ opacity: 0, rotate: 15, y: -6 }}
