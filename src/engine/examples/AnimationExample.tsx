...
export function AnimationExample() {
  ...
  const handleAnimationComplete = (targetName: string, animationType: string) => {
  ...
  const handleAnimationUpdate = (targetName: string, progress: number) => {
    ...
    if (progress % 0.25 < 0.01) {
      console.log(`Progreso de animación en ${targetName}: ${Math.round(progress * 100)}%`);
    }
  };
...
export function AdvancedAnimationExample() {
  ...
  const handleAnimationStart = (targetName: string, animationType: string) => {
    ...
    console.log('🎬 Animación iniciada:', { target: targetName, type: animationType });
  };

  const handleAnimationComplete = (targetName: string, animationType: string) => {
    // Ejecutar lógica post-animación
    console.log('✅ Animación completada:', { target: targetName, type: animationType });
  };
